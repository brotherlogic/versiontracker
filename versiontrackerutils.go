...
func (s *Server) validateVersion(ctx context.Context, name string) error {
	...
	if nv != nil && time.Since(time.Unix(nv.GetVersionDate(), 0)) > time.Hour*24*60 && config.BuildBugs[cv.GetJob().GetName()] == 0 {
		...
		issue, err := s.BounceImmediateIssue(ctx, cv.GetJob().GetName(), "Build needed", fmt.Sprintf("According to %v %v was last built %v", s.Registry.Identifier, cv.GetJob().GetName(), time.Unix(lv.GetVersionDate(), 0)), true, false)
		...
		if time.Since(time.Unix(nv.GetVersionDate(), 0)) <= time.Hour*24*60 {
			err = s.DeleteBounceIssue(ctx, val, cv.GetJob().GetName())
			if err != nil {
				return err
			}
